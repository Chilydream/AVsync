...
import numpy as np
import ipdb
from yolo_models import vlad_simple
...
class Resnet34(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(filters[3][2], vlad_centers + ghost_centers, kernel_size=(7, 1), bias=True)

        self.cluster = nn.Parameter(data=torch.Tensor(vlad_centers + ghost_centers, 512), requires_grad=True)

        #
        proj = 512
        dropout = 0.9

        self.dense = nn.Linear(vlad_centers * 512, proj)
    ...
    def forward(self, x):
        ...
        # (batch_size, 512, 1, 16)
        return feat_broadcast


class MyResnet34NetVLAD(nn.Module):
    def __init__(self):
        super().__init__()
        self.encoder_resnet = Resnet34()
        self.vlad_layer = vlad_simple.NetVLAD()
        self.dense = torch.nn.Linear(5120, 512)

    def forward(self, x):
        res_output = self.encoder_resnet(x)
        # (batch, 512, 1, 16)

        vlad_output = self.vlad_layer(res_output)

        final_output = self.dense(vlad_output)
        return final_output
