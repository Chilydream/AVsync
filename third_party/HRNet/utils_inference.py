...
def get_model_by_name(model_name, root_models_path='hrnetv2_models', prefix='HR18-', model_type='landmarks',
	                  device='cuda'):
...
def get_lmks_by_img(model, img, output_size=(256, 256), rot=0):
	...
	with torch.no_grad():
		pred = model(img_crop)
	return decode_preds(pred, [face_center], [crop_scale], [output_size[0]/4, output_size[1]/4]).cpu().numpy().squeeze(
		0)


def get_batch_lmks(model, image_batch, output_size=(256, 256)):
	# img.shape=(batch, 3, width, height)
	face_center = torch.Tensor([[image_batch.shape[3]//2, image_batch.shape[2]/2]]*image_batch.shape[0])
	crop_scale = [max((image_batch.shape[3])/output_size[0], (image_batch.shape[2])/output_size[1])]*image_batch.shape[
		0]

	# img_crop = crop(img_batch, face_center, crop_scale, output_size=output_size, rot=rot)
	# 输入输出都是（256，256）不用 crop
	# 输入的是tensor的话，能用下面这个操作吗
	image_batch = image_batch/255.0
	image_batch[:, 0, ...] = (image_batch[:, 0, ...]-0.485)/0.229
	image_batch[:, 1, ...] = (image_batch[:, 1, ...]-0.456)/0.224
	image_batch[:, 2, ...] = (image_batch[:, 2, ...]-0.406)/0.225
	with torch.no_grad():
		pred = model(image_batch)
	# return  (b, 68, 2)
	return decode_preds(pred, face_center, crop_scale, [output_size[0]/4, output_size[1]/4])
